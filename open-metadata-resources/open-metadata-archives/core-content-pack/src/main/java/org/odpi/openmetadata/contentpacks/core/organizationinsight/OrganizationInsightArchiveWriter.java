/* SPDX-License-Identifier: Apache-2.0 */
/* Copyright Contributors to the ODPi Egeria project. */
package org.odpi.openmetadata.contentpacks.core.organizationinsight;

import org.odpi.openmetadata.contentpacks.core.ContentPackDefinition;
import org.odpi.openmetadata.contentpacks.core.base.ContentPackBaseArchiveWriter;
import org.odpi.openmetadata.contentpacks.core.core.CorePackArchiveWriter;
import org.odpi.openmetadata.repositoryservices.connectors.stores.archivestore.properties.OpenMetadataArchive;


/**
 * OrganizationInsightArchiveWriter creates an open metadata archive that includes the connectors that
 * gather organization insight.
 */
public class OrganizationInsightArchiveWriter extends ContentPackBaseArchiveWriter
{
    /**
     * Default constructor initializes the archive.
     */
    public OrganizationInsightArchiveWriter()
    {
        super(ContentPackDefinition.ORGANIZATION_INSIGHT_CONTENT_PACK.getArchiveGUID(),
              ContentPackDefinition.ORGANIZATION_INSIGHT_CONTENT_PACK.getArchiveName(),
              ContentPackDefinition.ORGANIZATION_INSIGHT_CONTENT_PACK.getArchiveDescription(),
              ContentPackDefinition.ORGANIZATION_INSIGHT_CONTENT_PACK.getArchiveFileName(),
              new OpenMetadataArchive[]{new CorePackArchiveWriter().getOpenMetadataArchive()});
    }


    /**
     * Implemented by subclass to add the content.
     */
    @Override
    public void getArchiveContent()
    {
        /*
         * Create the default governance engines
         */
        super.createGovernanceEngines(ContentPackDefinition.ORGANIZATION_INSIGHT_CONTENT_PACK);

        /*
         * Register the governance services that are going to be in the default governance engines.
         */
        super.createGovernanceServices(ContentPackDefinition.ORGANIZATION_INSIGHT_CONTENT_PACK);

        /*
         * Connect the governance engines to the governance services using the request types.
         */
        super.createRequestTypes(ContentPackDefinition.ORGANIZATION_INSIGHT_CONTENT_PACK);

        /*
         * Link the Lovelace services to the Babbage Analytical Engine.
         */
        super.addLovelaceCatalogTargets(ContentPackDefinition.ORGANIZATION_INSIGHT_CONTENT_PACK);

        /*
         * Define the solution components for this solution.
         */
        this.addSolutionBlueprints(ContentPackDefinition.ORGANIZATION_INSIGHT_CONTENT_PACK);
        this.addSolutionLinkingWires(ContentPackDefinition.ORGANIZATION_INSIGHT_CONTENT_PACK);

        /*
         * Saving the GUIDs means that the guids in the archive are stable between runs of the archive writer.
         */
        archiveHelper.saveGUIDs();
        archiveHelper.saveUsedGUIDs();
    }


    /**
     * Main program to initiate the archive writer for the connector types for data store connectors supported by
     * the Egeria project.
     *
     * @param args ignored
     */
    public static void main(String[] args)
    {
        try
        {
            OrganizationInsightArchiveWriter archiveWriter = new OrganizationInsightArchiveWriter();
            archiveWriter.writeOpenMetadataArchive();
        }
        catch (Exception error)
        {
            System.err.println("Exception: " + error);
            System.exit(-1);
        }
    }
}