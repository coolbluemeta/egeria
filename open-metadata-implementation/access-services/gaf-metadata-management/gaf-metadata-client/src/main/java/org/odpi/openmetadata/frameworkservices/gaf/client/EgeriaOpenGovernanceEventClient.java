...
import org.odpi.openmetadata.frameworkservices.gaf.connectors.outtopic.GAFOutTopicClientConnector;
 ...
 */
public class EgeriaOpenGovernanceEventClient extends OpenMetadataEventClient
{
    private static final String        serviceName = AccessServiceDescription.GAF_METADATA_MANAGEMENT.getServiceName();
    private final        OCFRESTClient restClient;               /* Initialized in constructor */

    private final InvalidParameterHandler invalidParameterHandler = new InvalidParameterHandler();

    private GAFOutTopicClientConnector configurationEventTopicConnector = null;
     ...
     */
    public EgeriaOpenGovernanceEventClient(String   serverName,
                                                                            ...
                                                                            UserNotAuthorizedException
    {
        ...
        final String   urlTemplate = serverPlatformURLRoot + "/servers/{0}/open-metadata/access-services/open-governance-service/users/{1}/topics/out-topic-connection/{2}";
        ...
        if (configurationEventTopicConnector == null)
        {
            try
            {
                ...
                if (connector == null)
                {
                    throw new PropertyServerException(OpenGovernanceErrorCode.NULL_CONNECTOR_RETURNED.getMessageDefinition(topicConnection.getQualifiedName(),
                                                                                                                           serviceName,
                                                                                                                           serverName,
                ...
                if (connector instanceof GAFOutTopicClientConnector)
                {
                    configurationEventTopicConnector = (GAFOutTopicClientConnector) connector;
                    configurationEventTopicConnector.start();
                }
                else
                {
                    throw new PropertyServerException(OpenGovernanceErrorCode.WRONG_TYPE_OF_CONNECTOR.getMessageDefinition(topicConnection.getQualifiedName(),
                                                                                                                        serviceName,
                                                                                                                        serverName,
                                                                                                                        serverPlatformURLRoot,
            ...
            {
                throw new PropertyServerException(OpenGovernanceErrorCode.UNEXPECTED_EXCEPTION.getMessageDefinition(connectorError.getClass().getName(),
                                                                                                                 methodName,
                                                                                                                 serviceName,
            ...
            catch (Exception error)
            {
                if (auditLog != null)
                {
                   auditLog.logException(methodName,
                                         OpenGovernanceAuditCode.EVENT_SHUTDOWN_ERROR.getMessageDefinition(error.getClass().getName(),
                                                                                                           error.getMessage()),
                                         error);
