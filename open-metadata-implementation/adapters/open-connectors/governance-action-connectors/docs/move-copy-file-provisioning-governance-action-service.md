...
If lineage is requested, it includes adding a new [DataFile](https://egeria-project.org/types/2/0220-Files-and-Folders)
asset for the new file in the destination folder and creating [DataFlow](https://egeria-project.org/types/7/0750-Data-Passing)
...
* An [integration connector](https://egeria-project.org/concepts/integration-connector) may publish the lineage to a third party.

* Additional [Governance Action Services](https://egeria-project.org/concepts/governance-action-service) may be triggered to provide additional enrichment of the lineage graph.
...
This is its connection definition to use when creating the definition of the governance action service using the [Open Governance Service](https://egeria-project.org/services/gaf-metadata-management). Remove the configuration properties that are not required. Replace `{{processName}}` and `{{pattern}}` as required. 
...
When this governance action service is called through an [engine action](https://egeria-project.org/concepts/engine-action) it supports the following options.
...
- [Designing a Provisioning Governance Action Service](https://egeria-project.org/guides/developer/governance-action-services/provisioning-governance-service)
