...
![Figure 1](apache-atlas-catalog-integration-connector.png)
...
* If the instance was originally created in Apache Atlas, it is owned by Apache Atlas.  When the instance is copied into the open metadata ecosystem, its [provenance](https://egeria-project.org/features/metadata-provenance/overview) is set to indicate that this Apache Atlas server is its origin, which effectively makes it read-only in the open metadata ecosystem.
...
#### Example of an Apache Atlas glossary category
...
* Uses an [external identifier](https://egeria-project.org/features/external-identifiers/overview) attached to the open metadata instance to hold the unique identifier (GUID) of the equivalent Apache Atlas metadata instance as well as using [metadata provenance](https://egeria-project.org/features/metadata-provenance/overview) to identify the origin of instances being synchronized.  The external identifier is returned in the 

    * *correlationProperties* returns the external identifier.
    * *origin* of the *elementHeader* shows the metadata provenance.

#### Example of an open metadata glossary"
...
The Apache Atlas integration connector runs in an [Integration Daemon](https://egeria-project.org/concepts/integration-daemon).  There are two parts to the configuration:
...
The [connection](https://egeria-project.org/concepts/connection) object provides information about configurations properties and the endpoint where Apache Atlas is running. 
...
#### Using secrets for Apache Atlas's userId and password
...
* *Static configuration* by adding it directly to the [configuration document](https://egeria-project.org/concepts/configuration-document) for the Integration Daemon.  This creates a static configuration that requires a restart of the Integration Daemon to change the configuration.
* *Dynamic configuration* by adding it to an open metadata [integration group](https://egeria-project.org/concepts/integration-group).  The integration group is stored in the open metadata repository of a [Metadata Access Store](https://egeria-project.org/concepts/metadata-access-store). The associate Integration Daemon(s) immediately adapts what they are running as changes are made to the integration group contents.
