...
The Apache Atlas Survey Action Service builds a [survey report](https://egeria-project.org/concepts/survey-report) that describes the types defined in the Apache Atlas server and the numbers of instances that are found for these types.

![Figure 1](apache-atlas-survey-action-service.png)
...
1. Measure Resource - Retrieves the overall metrics from the Apache Atlas server.  These are stored in a [*ResourceMeasureAnnotation*](https://egeria-project.org/types/0660-Resource-Measures) entity linked to the [*SurveyReport*](https://egeria-project.org/types/6/0603-Survey-Reports) entity generated for each run of the Apache Atlas Survey Action Service.
2. Schema Extraction - Retrieves the types from Apache Atlas and organizes them in a linked graph of [*Schema Attributes*](https://egeria-project.org/types/5/0533-Graph-Schemas) entities.  All the graph schema attributes are linked to a [*GraphSchemaType*](https://egeria-project.org/types/5/0533-Graph-Schemas) entity which is in turn linked to the [*SurveyReport*](https://egeria-project.org/types/6/0603-Survey-Reports) entity.
3. Profile Data - Retrieves each entity in the Apache Atlas server and adds the following counts to [*ResourceProfileAnnotation*](https://egeria-project.org/types/6/0620-Resource-Profiling) entities linked from the appropriate data field entities:
...
Prior to running the Apache Atlas Survey Action Service, an [asset](https://egeria-project.org/concepts/asset) and [connection](https://egeria-project.org/concepts/connection) must be created for the Apache Atlas server that is to be analysed.

Typically, the asset for Apache Atlas is of type [*SoftwareServer*](https://egeria-project.org/types/0/0040-Software-Server) with a *deployedImplementationType* set to *Apache Atlas Server*.  The `networkAddress` in the connection's [endpoint](https://egeria-project.org/concepts/endpoint) is the hostname and port of the Apache Atlas server.  For example, `http://localhost:21000`.

![Figure 2](apache-atlas-survey-action-service-setup.png
)
> **Figure 2:** Metadata added to the open metadata repository

## Survey Reports

Each time the Apache Atlas Survey Action Service runs, there is a new [survey report](https://egeria-project.org/concepts/survey-report) created.

![Figure 3](apache-atlas-survey-action-service-reports.png)
...
![Figure 4](apache-atlas-survey-action-service-analysis.png)
...
* A [*GraphVertex*](https://egeria-project.org/types/5/0533-Graph-Schemas) entity is created for each Apache Atlas entity type, business metadata type and classification type.  
* A [*GraphEdge*](https://egeria-project.org/types/5/0533-Graph-Schemas) entity is created for each Apache Atlas relationship type, and each permitted use of a classification type by an entity type.

*GraphVertex* and *GraphEdge* are types of [*SchemaAttribute*](https://egeria-project.org/types/5/0505-Schema-Attributes).  The graph schema attributes are connected together using [*GraphEdgeLink*](https://egeria-project.org/types/5/0533-Graph-Schemas) relationships that connect each GraphEdge schema attribute to two GraphVertex schema attributes.
...
All the graph vertices are linked to a [*GraphSchemaType*](https://egeria-project.org/types/5/0533-Graph-Schemas) entity using the [*AttributeForSchema*](https://egeria-project.org/types/5/0505-Schema-Attributes) relationship.  The *GraphSchemaType* entity is linked to the asset for the Apache Atlas server using the [Schema](https://egeria-project.org/types/5/0503-Asset-Schema/) relationship.

![Figure 5](apache-atlas-schema-extraction.png)
...
![Figure 6](apache-atlas-survey-action-service-profile.png)
